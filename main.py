...
def generate_requirements(project_folder, add_last_versions=False):
    all_imports = set()

    # Путь к .gitignore в выбранной папке
    gitignore_path = os.path.join(project_folder, '.gitignore')
    
    # Если .gitignore отсутствует в выбранной папке, используем встроенный
    ...
    external_imports = all_imports - known_std_libs
...
def select_folder_and_generate_requirements(add_last_versions):
    root = tk.Tk()
    root.withdraw()  # Скрыть основное окно tkinter

    folder = filedialog.askdirectory(title="Выберите папку с проектом")
    if folder:
        generate_requirements(folder, add_last_versions)
...
if __name__ == "__main__":
    ...
    select_folder_and_generate_requirements(args.add_last_versions)
